...
#### T5: Message Reorder, Replay, or Modification
  ...
  To prevent from reorder, replay, or modification attacks, we will be using a few different protocols. First, for reorder and modification, we will be utilizing HMAC and TLS like shared key generation protocols. When the client and servers connect, they will both generate 128 byte value pseudorandom numbers (Ra, Rb) generated by the Java Secure Random. These two numbers will be concatenated and hashed to create 2 separate session keys. Key 1 is SHA256(Ra||Rb) and Key 2 is SHA256(Rb||Ra). Key 1 will be used for encryption, while Key 2 will be used for shared signing and verification. SHA256 was used because it outputs a fixed 32 byte output that can be used like an AES key. Hashing is also extremely quick relative to generating other keys and relatively secure. If adversary A were to impersonate the group server and try to replay messages, they would not be able to decrypt any prior messages or any future messages since keys are generated each session.
Hence it prevents forward and backward attacks.
  
When a message is sent, whether it is a request from a client or a response form a server, the message will be of the following general structure:

  {req/resp||token||padding}Ka, HMAC(Kb, {req/resp||token||padding})

  That is to say, the request or response is concatenated with the token (if necessary), and padding and that entire concatenation is encrypted with the first shared key. Then, the HMAC of the encrypted data is calculated with the second shared key. Signing each transaction is not necessary because the keys are generated by using pseudorandom secrets only shared between the client and server. The receiver can then verify that the message hasn't been replayed or modified using the HMAC and the pseudorandom generated shared keys.
  ...
  Our group chose to use TLS-like shared keys and sequence numbers because mostly because of ease of implementation and efficiency. The use of shared symmetric keys is much faster than using RSA for everything, and the addition and checking of a simple sequence number is very lightweight and quick. RSA is used to as an initial identification and verification mechanism to ensure that the communication is legitimate. Limiting the use of RSA public keys are also important so that messages cannot be compared against eachother offline. By limiting public keys to encrypting pseudo-random numbers, it ensures uncertainty in the system as much as the system allows.

  These protocols are effective because both parties will agree on the keys, the keys will only be known to the client/server sharing them, and the keys will be generated anew on each subsequent connection. Each party only receives a partial key from the alternate party and generates the key independently. Both parties contribute to the randomness of the key generation which is a stronger approach than a one party contribution. Stolen keys cannot be used in a new session, replay attacks cannot occur, and each thread maintains its own expected sequence numbers for checking, so outside interference is unlikely.



### T6: File Leakage

  We are to assume that the File Server will leak files to users that are not members of the groups that are approved to view them. Then, it is necessary for the system to create per-group keys for encrypting each file, and it is necessary to update those keys when the members of a group change. Regenerating and batch re-encrypting each file is extremely cumbersome, plus, it requires the group server to talk to the file servers directly, which is not possible in our system.
